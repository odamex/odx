export const TAG_ID = 0xAD0;

export const PROTOCOL_VERSION = 9;
export function VERSIONMAJOR(V: number) { return Math.floor(V / 256); }
export function VERSIONMINOR(V: number) { return Math.floor((V % 256) / 10); }
export function VERSIONPATCH(V: number) { return Math.floor((V % 256) % 10); }
export function VERSION() { return Math.floor(0 * 256 + (PROTOCOL_VERSION*10)); }

export namespace OdalPapi {

	export const MASTER_CHALLENGE = 777123;
	export const MASTER_RESPONSE  = 777123;
	export const SERVER_CHALLENGE = 0xAD011002;
	export const SERVER_VERSION_CHALLENGE = 0xAD011001;
	export const PING_CHALLENGE = 1;

	export interface MasterResponse {
		ip: string;
		port: number;
	}

	export enum CvarType {
		CVARTYPE_NONE = 0, // Used for no sends
		CVARTYPE_BOOL,
		CVARTYPE_BYTE,
		CVARTYPE_WORD,
		CVARTYPE_INT,
		CVARTYPE_FLOAT,
		CVARTYPE_STRING,

		CVARTYPE_MAX = 255
	}

	export enum GameType {
		GT_Cooperative = 0,
		GT_Deathmatch,
		GT_TeamDeathmatch,
		GT_CaptureTheFlag,
		GT_Survival,
		GT_Horde,
		GT_Max
	}

	export interface Cvar {
		name: string;
		value: string;
		cType: CvarType;

		i32?: number;
		ui32?: number;
		i16?: number;
		ui16?: number;
		i8?: number;
		ui8?: number;
		b?: boolean;
	}

	export interface Team {
		name: string;
		color: number;
		score: number;
	}

	export interface Player	{
		name: string;
		color: number;
		kills: number;
		deaths: number;
		time: number;
		frags: number;
		ping: number;
		team: number;
		spectator: boolean;
	}

	export interface Wad {
		name: string;
		hash: string;
	}

	export class ServerInfo {
		address: MasterResponse = {ip: '', port: 0};
		patches: string[] = [];
		cvars: Cvar[] = [];
		teams: Team[] = [];
		wads: Wad[] = [];
		players: Player[] = [];
		name: string | null = null;
		passwordHash: string | null = null;
		currentMap: string | null = null;
		versionRevStr: string | null = null;
		gameType: GameType = 0;
		response: number | null = null;
		versionRevision: number | null = null;
		versionProtocol: number | null = null;
		versionRealProtocol: number | null = null;
		pTime: number | null = null;
		scoreLimit: number | null = null;
		timeLimit: number | null = null;
		timeLeft: number | null = null;
		versionMajor: number | null = null;
		versionMinor: number | null = null;
		versionPatch: number | null = null;
		maxClients: number | null = null;
		maxPlayers: number | null = null;
		lives: number | null = null;
		sides: number | null = null;
		responded = false;

		ping = 0;

		constructor() {}
	}
}
